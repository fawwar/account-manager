...
// use pattern containing LOGGER_LOG_FILE_PATTERN_DATE if you want to append log
// to the same per-day log file
...
public:
    bool configure_with(const boost::filesystem::path& pathConfig,
        const char* szSection = LOGGER_CONFIG_INI_SECTION);

protected:
    virtual void load_common_conf(
        const gorilla::utility::ini_file& ini, const char* szSection);
    virtual void load_file_conf(
        const gorilla::utility::ini_file& ini, const char* szSection);
    virtual void load_console_conf(
        const gorilla::utility::ini_file& ini, const char* szSection);
    virtual void load_debugger_conf(
        const gorilla::utility::ini_file& ini, const char* szSection);
    virtual void load_syslog_conf(
        const gorilla::utility::ini_file& ini, const char* szSection);
