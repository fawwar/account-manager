...
20050420 Seanni
	In SHA1Final, the line "memcpy (digest, ctx->iv, sizeof (digest));"
	has no effect since digest will be completely overwritten in following
	loop.
	If the line memcpy was used, sizeof(digest) may be incorrect, and should be 
	replaced by 20. 

20050117 Seanni
	Initial version created, from libmd

Description:
	
	This is actually SHA-1
	LIB-CROSS

Note:
	Result compared with openssl's sha1 under Linux: ok
...
/*
   ...
   SHA spec.
*/
static void shaByteSwap(SHA_UI32 *dest, const SHA_BYTE *src, unsigned int words)
{
	while(words--)
	{
		*dest++ = (SHA_UI32) ((unsigned) src[0] << 8 | src[1]) << 16 |
 ...
 */
static void SHATransform(SHAContext *sha)
{
	register SHA_UI32 A, B, C, D, E;
#if SHA_VERSION
	register SHA_UI32 t;
#endif
...
void SHA1Update (SHAContext * ctx, const unsigned char *buf, unsigned int len)
{
	unsigned i;
	...
	{
		memcpy ((SHA_BYTE *) ctx->key + i, buf, len);
		return;
	}

	if (i)
	{				/* First chunk is an odd size */
		memcpy ((SHA_BYTE *) ctx->key + i, buf, SHA_BLOCKBYTES - i);
		shaByteSwap (ctx->key, (SHA_BYTE *) ctx->key, SHA_BLOCKWORDS);
		SHATransform (ctx);
...
void SHA1Final (SHAContext *ctx, unsigned char digest[20])
{
#if HAVE64
...
#endif
	SHA_BYTE *p = (SHA_BYTE *) ctx->key + i;	/* First unused byte */
	SHA_UI32 t;
	...
	*p++ = 0x80;
	...
	{				/* Padding forces an extra block */
		memset (p, 0, i);
		shaByteSwap (ctx->key, (SHA_BYTE *) ctx->key, 16);
		SHATransform (ctx);
		p = (SHA_BYTE *) ctx->key;
		i = 64;
	}
	memset (p, 0, i - 8);
	shaByteSwap (ctx->key, (SHA_BYTE *) ctx->key, 14);

	/* Append length in bits and transform */
#if HAVE64
	ctx->key[14] = (SHA_UI32) (ctx->bytes >> 29);
	ctx->key[15] = (SHA_UI32) ctx->bytes << 3;
#else
	...
	{
		t = ctx->iv[i];
		digest[i * 4 + 0] = (SHA_BYTE) (t >> 24);
		digest[i * 4 + 1] = (SHA_BYTE) (t >> 16);
		digest[i * 4 + 2] = (SHA_BYTE) (t >> 8);
		digest[i * 4 + 3] = (SHA_BYTE) t;
	}

	memset(ctx, 0, sizeof(SHAContext)); 			/* In case it's sensitive */
}
